...
export function useAuctions() {
  ...
  const allAuctions: Auction[] = useMemo(() => {
    const auctions =
      data?.bm016AuctionModels?.edges?.map((edge) => {
  ...
  const allAuctionItems: AuctionItem[] = useMemo(() => {
    return data?.bm016AuctionItemModels?.edges?.map((edge) => edge.node) || [];
  }, [data]);

  const allBids: Bid[] = useMemo(() => {
    return (
      data?.bm016BidModels?.edges?.map((edge) => {
        const bid = edge.node;
        return {
          ...bid,
          bidder: bid.bidder
  ...
  const allOffers: Offer[] = useMemo(() => {
    return (
      data?.bm016OfferModels?.edges?.map((edge) => {
