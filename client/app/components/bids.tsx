...
export default function Bids({
  ...
  const renderContent = () => {
          ...
          <section className="mx-auto w-full max-w-6xl overflow-hidden rounded-2xl border border-[rgb(50,255,52)]/80 bg-black/55 shadow-[0_16px_40px_rgba(5,20,5,0.35)]">
            <div className="grid gap-8 p-6 grid-cols-[minmax(0,0.35fr)_minmax(0,0.65fr)] md:items-start">
              <div className="flex flex-col items-center gap-4 text-center md:items-start md:text-left">
                ...
                <span
                  className={`inline-flex items-center justify-center rounded-full px-3 py-1 text-[11px] font-orbitron uppercase tracking-[0.16em] ${getStatusStyle(selectedCollection.status)}`}
                >
                ...
                <p className="text-xs leading-relaxed text-[rgb(186,255,188)]/70">
                  ...
                  {selectedCollection.totalMonsters === 1 ? "NFT" : "NFTs"} in
                  this collection
                </p>
                <p className="text-xs leading-relaxed text-[rgb(186,255,188)]/70">
                  Seller:{" "}
    ...
    const selectedRow = row1.some((c) => c.id === selectedCollectionId)
      ? 1
      : row2.some((c) => c.id === selectedCollectionId)
        ? 2
        : row3.some((c) => c.id === selectedCollectionId)
          ? 3
          : null;
